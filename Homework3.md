...
&emsp;&emsp;重构代码之后,Mesh被封装成了一个单独的类，Mesh中的节点，单元格，边的数量和各种用于存储具体信息的vector成为了类中的私有成员。原有代码中的对Mesh的各种操作被划分成了类中的多个成员函数。这些成员函数的主要功能有：从不同类型的文件中加载或者写入Mesh，着色器和具体对Mesh进行计算操作的kernel function  
